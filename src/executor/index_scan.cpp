...
IndexScan::IndexScan(Table* table, BTreeIndex* index, std::vector<size_t> projected,
                     std::optional<Bound> lo, std::optional<Bound> hi)
    : table_(table), index_(index), projected_(std::move(projected)), lo_(std::move(lo)),
      hi_(std::move(hi)) {
...
static void append_from_column(ColumnVector& cv, ColumnFile& cf, u64 row_id) {
    ...
    switch (cf.type()) {
    case TypeId::INT32:
        cv.append_i32(cf.get_i32(row_id).value());
        break;
    case TypeId::INT64:
        cv.append_i64(cf.get_i64(row_id).value());
        break;
    case TypeId::DOUBLE:
        cv.append_f64(cf.get_f64(row_id).value());
        break;
    case TypeId::VARCHAR:
        cv.append_str(cf.get_str(row_id));
        break;
    case TypeId::BOOL:
        cv.append_bool(cf.get_bool(row_id).value());
        break;
    case TypeId::DATE:
        cv.append_date(Date{cf.get_date(row_id).value()});
        break;
    case TypeId::TIMESTAMP:
        cv.append_timestamp(Timestamp{cf.get_timestamp(row_id).value()});
        break;
    default:
        break;
    }
