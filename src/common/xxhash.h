...
inline u64 read64(const byte* p) { u64 v; std::memcpy(&v, p, 8); return v; }
inline u32 read32(const byte* p) { u32 v; std::memcpy(&v, p, 4); return v; }
...
inline u64 xxhash64(const byte* data, usize len, u64 seed = 0) {
    ...
    if (len >= 32) {
        ...
        do {
            v1 = detail::xxh_round(v1, detail::read64(p)); p += 8;
            v2 = detail::xxh_round(v2, detail::read64(p)); p += 8;
            v3 = detail::xxh_round(v3, detail::read64(p)); p += 8;
            v4 = detail::xxh_round(v4, detail::read64(p)); p += 8;
        } while (p <= limit);

        h64  = detail::rotl64(v1,  1) + detail::rotl64(v2,  7)
    ...
    h64 ^= h64 >> 33; h64 *= detail::XXH_PRIME2;
    h64 ^= h64 >> 29; h64 *= detail::XXH_PRIME3;
    h64 ^= h64 >> 32;
