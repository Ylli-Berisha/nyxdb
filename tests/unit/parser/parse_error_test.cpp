...
TEST(ParseErrorTest, RenderLine1Col1) {
    ...
    SourceLoc loc{0, 6};
    auto msg = render_source_error(src, loc, "parse error", "hello");
...
TEST(ParseErrorTest, CaretPositionMidLine) {
    ...
    SourceLoc loc{7, 2};
    auto msg = render_source_error(src, loc, "parse error", "err");
...
TEST(ParseErrorTest, LineTwoAfterNewline) {
    ...
    SourceLoc loc{6, 5};
    auto msg = render_source_error(src, loc, "parse error", "boom");
...
TEST(ParseErrorTest, SnippetContainsOnlyOffendingLine) {
    ...
    SourceLoc loc{6, 5};
    auto msg = render_source_error(src, loc, "parse error", "x");
...
TEST(ParseErrorTest, ZeroLengthLocRendersSingleCaret) {
    std::string src = "abc";
    SourceLoc loc{3, 0};
    auto msg = render_source_error(src, loc, "parse error", "eof");
